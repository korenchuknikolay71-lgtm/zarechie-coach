 ...
 */
export async function buildTeamPlaybook(roster) {
  ...
  const zResults = await redisPipeline(
    players.map(p => ['ZREVRANGE', `coach:sessions:${p.id}`, 0, 59])
  );
  ...
  players.forEach((p, pi) => {
    ...
    for (const d of dates) {
      sessionCmds.push(['get', `coach:session:${p.id}:${d}`]);
      sessionIndex.push({ playerIdx: pi, date: d });
    }
  });

  const neuroCmds = players.map(p => ['get', `neuro:history:${p.id}`]);
...
/**
 * getTeamPlaybook()
 * Reads cached playbook from coach:playbook. Returns null if not cached.
 */
export async function getTeamPlaybook() {
  const raw = await redis('get', 'coach:playbook').catch(() => null);
  return parseJSON(raw);
}

/**
 * refreshTeamPlaybook(roster)
 * Rebuilds the playbook and caches it at coach:playbook with a 7-day TTL.
 * Returns the freshly built playbook.
 */
export async function refreshTeamPlaybook(roster) {
  const playbook = await buildTeamPlaybook(roster);
  await redis('set', 'coach:playbook', JSON.stringify(playbook), 'EX', 604800).catch(() => null);
  return playbook;
}
